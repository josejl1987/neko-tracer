...
BRESULT scrnmng_create(UINT8 scrnmode) {
	...
	if (scrnmode & SCRNMODE_FULLSCREEN) {
		...
		//	ハードウェアアクセラレーションの使用・未使用切り替え関連
		CheckMenuItem(hmenu, IDM_WINDOW_SOFT, MF_UNCHECKED);
		//
...
//		CheckMenuItem(hmenu, IDM_WINDOW, MF_CHECKED);
		if (scrnmode & SCRNMODE_DDRAWHARD) {
			CheckMenuItem(hmenu, IDM_WINDOW, MF_CHECKED);
			CheckMenuItem(hmenu, IDM_WINDOW_SOFT, MF_UNCHECKED);
		}
		else {
			CheckMenuItem(hmenu, IDM_WINDOW, MF_UNCHECKED);
			CheckMenuItem(hmenu, IDM_WINDOW_SOFT, MF_CHECKED);
		}
...
//	}
	if (scrnmode & SCRNMODE_DDRAWHARD) {
		...
		if (DirectDrawCreate((LPGUID)DDCREATE_EMULATIONONLY, &ddraw.ddraw1, NULL) != DD_OK) {
			goto scre_err;
		}
	}
